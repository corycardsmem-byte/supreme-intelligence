...
app.get('/health', (req, res) => {
  res.json({ ok: true, service: 'supreme-intelligence-api', status: 'operational' });
});
...
app.post('/chat', async (req, res) => {
  try {
    ...
    const evidence = createEvidenceRecord({ answer: response.answer, sources, score: governance.score });

    return res.json({ answer: response.answer, sources: response.sources, governance, evidence });
  } catch (error) {
    return res.status(500).json({ error: 'Unable to process request.', detail: error.message });
  }
});
