import { CRA_SPEC, canonicalize, canonicalHash, normalizeTransition, verifyTransition } from './symbolic-spec.js';
...
export const NODE_TYPES = Object.freeze({
  ...
  EXECUTION_PLAN: 'EXECUTION_PLAN', EXECUTION_RESULT: 'EXECUTION_RESULT',
  AGENT_ASSERTION: 'AGENT_ASSERTION', HYPOTHESIS: 'HYPOTHESIS', OBSERVATION: 'OBSERVATION',
  CONTRADICTION: 'CONTRADICTION', SOURCE: 'SOURCE', TOOL_RESULT: 'TOOL_RESULT',
  MODEL_OUTPUT: 'MODEL_OUTPUT', EVALUATION: 'EVALUATION'
});
...
export const EDGE_TYPES = Object.freeze({
  ...
  DESCRIBES: 'DESCRIBES', VERIFIED_BY: 'VERIFIED_BY', SUPERSEDES: 'SUPERSEDES',
  OBSERVED_FROM: 'OBSERVED_FROM', TRIGGERS: 'TRIGGERS', GENERATES: 'GENERATES',
  OUTPUT_OF: 'OUTPUT_OF', EVALUATES: 'EVALUATES'
});
...
function validateStateVectorPayload(payload) {
  if (typeof payload.propositionId !== 'string') throw new TypeError('STATE_VECTOR requires propositionId.');
  if (!CRA_SPEC.stateSpace.A.includes(payload.provenance) || !CRA_SPEC.stateSpace.E.includes(payload.evidence)) throw new TypeError('STATE_VECTOR contains invalid CRA A or E values.');
  const causal = payload.causal ?? CRA_SPEC.stateSpace.C[0];
  if (payload.causal === CRA_SPEC.causalNotApplicable) {
    if (payload.boundaryId != null || payload.causalBoundary != null) throw new TypeError('Ø STATE_VECTOR must omit any boundary reference.');
    return;
  }
  if (!CRA_SPEC.stateSpace.C.includes(causal)) throw new TypeError('STATE_VECTOR contains invalid causal value.');
  if (typeof payload.boundaryId !== 'string' || payload.boundaryId.length === 0) throw new TypeError('Causal STATE_VECTOR requires boundaryId.');
}
...
function assertNodeShape(type, payload) {
  ...
  if (type === NODE_TYPES.STATE_VECTOR) validateStateVectorPayload(payload);
  if (type === NODE_TYPES.PROOF_ARTIFACT && typeof payload.result !== 'string') throw new TypeError('PROOF_ARTIFACT requires result.');
  if (type === NODE_TYPES.EXECUTION_PLAN && typeof payload.objective !== 'string') throw new TypeError('EXECUTION_PLAN requires objective.');
  if (type === NODE_TYPES.EXECUTION_RESULT && (payload.planHash == null || typeof payload.planHash !== 'string')) throw new TypeError('EXECUTION_RESULT requires planHash.');
}
...
export function stateVector(payload, parents = []) {
  ...
  if (normalized.causal === CRA_SPEC.causalNotApplicable && (payload.boundaryId != null || payload.causalBoundary != null)) {
    throw new TypeError('Ø STATE_VECTOR must omit boundaryId.');
  }
  if (normalized.causal !== CRA_SPEC.causalNotApplicable && (typeof payload.boundaryId !== 'string' || payload.boundaryId.length === 0)) {
    throw new TypeError('Causal STATE_VECTOR requires boundaryId.');
  }
...
export class EpistemicMesh {
  ...
  createEdge(source, relation, target, metadata = {}) {
    if (!this.store.getNode(source) || !this.store.getNode(target)) throw new Error('Edges require existing nodes.');
    if (relation === EDGE_TYPES.SUPERSEDES && this.reachable(target, source, EDGE_TYPES.SUPERSEDES)) throw new Error('SUPERSEDES cycle rejected.');
    return this.store.putEdge(createEdge(source, relation, target, metadata));
  }
  createEvent(type, nodes, metadata = {}) { if (nodes.some((id) => !this.getNode(id))) throw new Error('Events require existing nodes.'); return this.store.putEvent(createEvent(type, nodes, metadata)); }
  getNode(id) { return this.store.getNode(id); }
  nodesByType(type) { return this.store.allNodes().filter((node) => node.nodeType === type); }
  edgesFrom(source, relation) { return this.store.allEdges().filter((edge) => edge.source === source && (!relation || edge.relation === relation)); }
  edgesTo(target, relation) { return this.store.allEdges().filter((edge) => edge.target === target && (!relation || edge.relation === relation)); }
  reachable(start, target, relation = EDGE_TYPES.SUPERSEDES) {
    const seen = new Set(); const queue = [start];
    while (queue.length) {
      const current = queue.shift();
      if (current === target) return true;
      for (const edge of this.edgesFrom(current, relation)) {
        if (!seen.has(edge.target)) { seen.add(edge.target); queue.push(edge.target); }
      }
    }
    return false;
  }
  stateHistory(propositionId) { return this.nodesByType(NODE_TYPES.STATE_VECTOR).filter((node) => node.payload.propositionId === propositionId); }
  ...
  descendantsOf(nodeId, limit = 10000) { const seen = new Set(); const queue = [nodeId]; while (queue.length && seen.size < limit) { const current = queue.shift(); for (const node of this.dependentsOf(current)) { if (!seen.has(node.id)) { seen.add(node.id); queue.push(node.id); } } } return [...seen].map((id) => this.getNode(id)); }
...
export async function commitVerifiedState(mesh, { propositionNode, evidenceNode, boundaryNode, transition, verification, previousState = null }) {
  ...
  const state = stateVector({
    propositionId: propositionNode.id,
    provenance: transition.provenance,
    evidence: transition.evidence,
    causal: transition.causalApplicable === false ? CRA_SPEC.causalNotApplicable : transition.causal,
    boundaryId: boundaryNode?.id ?? null,
    dependencyHashes: [propositionNode.id, ...(evidenceNode ? [evidenceNode.id] : [])],
    verificationId: proof.id,
    creationEventId: proofEvent.id,
    supersedes: previousState?.id ?? null
  }, [propositionNode.id, proof.id, ...(previousState ? [previousState.id] : [])]);
  ...
  if (evidenceNode) {
    if (transition.evidence === 'Contradicted') mesh.createEdge(evidenceNode.id, EDGE_TYPES.CONTRADICTS, propositionNode.id);
    else mesh.createEdge(evidenceNode.id, EDGE_TYPES.SUPPORTS, propositionNode.id);
  }
  if (boundaryNode) mesh.createEdge(propositionNode.id, EDGE_TYPES.EVALUATED_UNDER, boundaryNode.id);
